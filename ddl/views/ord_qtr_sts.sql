CREATE OR REPLACE VIEW ord_qtr_sts
(
  year,
  quarter,
  booked,
...
AS 
 SELECT rpt."year", rpt.quarter, sum(rpt.booked) AS booked, sum(rpt.confirmed) AS confirmed, sum(rpt.rescheduled) AS rescheduled, sum(rpt.completed) AS completed, sum(rpt.rejected) AS rejected, sum(rpt.cancelled) AS cancelled, sum(rpt.other) AS other, sum(rpt.booked + rpt.confirmed + rpt.rescheduled) AS "open", sum(rpt.booked + rpt.confirmed + rpt.rescheduled + rpt.completed) AS net, sum(rpt.rejected + rpt.cancelled) AS failed, sum(rpt.booked + rpt.confirmed + rpt.rescheduled + rpt.completed + rpt.rejected + rpt.cancelled + rpt.other) AS gross
   FROM ( SELECT map."year", map.quarter, sum(map.booked) AS booked, sum(map.confirmed) AS confirmed, sum(map.cancelled) AS cancelled, sum(map.completed) AS completed, sum(map.rejected) AS rejected, sum(map.rescheduled) AS rescheduled, sum(map.other) AS other
           FROM ( SELECT "date_part"('year'::text, cvt.created_at) AS "year", "date_part"('quarter'::text, cvt.created_at) AS quarter, cvt.booked, cvt.confirmed, cvt.cancelled, cvt.completed, cvt.rejected, cvt.rescheduled, cvt.other
          ...
          GROUP BY map."year", map.quarter
UNION 
         SELECT map."year", map.quarter, sum(map.booked) AS booked, sum(map.confirmed) AS confirmed, sum(map.cancelled) AS cancelled, sum(map.completed) AS completed, sum(map.rejected) AS rejected, sum(map.rescheduled) AS rescheduled, sum(map.other) AS other
           FROM ( SELECT "date_part"('year'::text, cvt.created_at) AS "year", "date_part"('quarter'::text, cvt.created_at) AS quarter, cvt.booked, cvt.confirmed, cvt.cancelled, cvt.completed, cvt.rejected, cvt.rescheduled, cvt.other
          ...
          GROUP BY map."year", map.quarter) rpt
  GROUP BY rpt."year", rpt.quarter
  ORDER BY rpt."year" DESC, rpt.quarter DESC;
